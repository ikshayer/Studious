...
export const PATCH = async (request, {params}) => {
    ...
    try {
        ...
        if(!user.endorsedBy.includes(endorsingUser)) return new Response('Endorsing User was not endorsed before the User before', {status: 400})

        await User.findByIdAndUpdate(params.id, {
            $inc: {endorsement: -1},
            $pull: {endorsedBy: endorsingUser}
        })
